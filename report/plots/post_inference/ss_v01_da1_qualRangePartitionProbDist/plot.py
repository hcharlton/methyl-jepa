...
aggs = [
    (
        ...
        .filter(pl.col("qual_slice_min") > i)
        .sum()
        / pl.col("qual_slice_min").filter(pl.col("qual_slice_min") > i).len()
...
chart = alt.Chart(df).mark_line().encode(
    ...
    alt.X('qual_cutoff').title('lower bound for min(qual[14:19])'),
).properties(
    width=600,
    height=600,
)
...
chart.save(pathlib.Path(__file__).with_name("ss_v01_da1_qualRangePartitionProbDist.svg"))